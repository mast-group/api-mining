...
public class MarkovChainTest {
	...
	public MarkovChainTest() {
		transitionMatrix = new Array2DRowRealMatrix(new double[][] { { 0.5, 0.5 }, { 0.25, 0.75 } });
		chain = new MarkovChain(transitionMatrix);
		stationaryVector = new ArrayRealVector(new double[] { 1.0 / 3.0, 2.0 / 3.0 });
	}
	...
	public void testExceptions() {
		...
		try {
			invalid = new MarkovChain(new Array2DRowRealMatrix(new double[][] { { 0.5 }, { 0.25 } }),
		...
		try {
			invalid = new MarkovChain(new Array2DRowRealMatrix(new double[][] { { 0.5, 0.5 }, { 0.25, 0.75 } }),
					new ArrayRealVector(new double[] { 0.5 }));
			Assert.fail("Expected DimensionMismatchException");
		} catch (final DimensionMismatchException e) {
		}
		try {
			invalid = new MarkovChain(new Array2DRowRealMatrix(new double[][] { { 0.5, -1 }, { 0.25, 0.75 } }),
					new ArrayRealVector(new double[] { 0.5, 0.5 }));
		...
		try {
			invalid = new MarkovChain(new Array2DRowRealMatrix(new double[][] { { 0.5, 0.5 }, { 0.25, 0.75 } }),
					new ArrayRealVector(new double[] { 0.5, -1 }));
			Assert.fail("Expected NotPositiveException");
		} catch (final NotPositiveException e) {
		}
		try {
			invalid = new MarkovChain(
					new Array2DRowRealMatrix(new double[][] { { 0.5, Double.POSITIVE_INFINITY }, { 0.25, 0.75 } }),
					new ArrayRealVector(new double[] { 0.5, 0.5 }));
		...
		try {
			invalid = new MarkovChain(new Array2DRowRealMatrix(new double[][] { { 0.5, 0.5 }, { 0.25, 0.75 } }),
					new ArrayRealVector(new double[] { 0.5, Double.POSITIVE_INFINITY }));
			Assert.fail("Expected MathIllegalArgumentException");
		} catch (final MathIllegalArgumentException e) {
		}
		try {
			invalid = new MarkovChain(new Array2DRowRealMatrix(new double[][] { { 0.0, 0.0 }, { 0.25, 0.75 } }),
					new ArrayRealVector(new double[] { 0.5, 0.5 }));
	...
	public void testGetStationaryVector_RealMatrix() {
		...
		Assert.assertArrayEquals(stationaryVector.toArray(), pi.toArray(), 1e-10);
		RealVector invalid = null;
		try {
			invalid = MarkovChain.getStationaryVector(new Array2DRowRealMatrix(new double[][] { { 0.5 }, { 0.25 } }));
			Assert.fail("Expected NonSquareMatrixException");
		} catch (final NonSquareMatrixException e) {
		}
		try {
			invalid = MarkovChain
					.getStationaryVector(new Array2DRowRealMatrix(new double[][] { { 0.5, -1 }, { 0.25, 0.75 } }));
			Assert.fail("Expected NotPositiveException");
		} catch (final NotPositiveException e) {
		}
		try {
			invalid = MarkovChain.getStationaryVector(
					new Array2DRowRealMatrix(new double[][] { { 0.5, Double.POSITIVE_INFINITY }, { 0.25, 0.75 } }));
			Assert.fail("Expected MathIllegalArgumentException");
		} catch (final MathIllegalArgumentException e) {
		}
		try {
			invalid = MarkovChain
					.getStationaryVector(new Array2DRowRealMatrix(new double[][] { { 0.0, 0.0 }, { 0.25, 0.75 } }));
			Assert.fail("Expected MathArithmeticException");
		} catch (final MathArithmeticException e) {
		}
		try {
			invalid = MarkovChain
					.getStationaryVector(new Array2DRowRealMatrix(new double[][] { { 1.0, 0.0 }, { 0.0, 1.0 } }));
	...
	public void testGetTransitionMatrix() {
		...
		for (int row = 0; row < tm.getColumnDimension(); row++) {
			Assert.assertArrayEquals(transitionMatrix.getData()[row], tm.getRow(row), 0);
		}
	}
	...
	public void testGetStationaryVector_0args() {
		Assert.assertArrayEquals(stationaryVector.toArray(), chain.getStationaryVector().toArray(), 1e-10);
	}
	...
	public void testSetState_RealVector() {
		...
		try {
			chain.setState(new ArrayRealVector(new double[] { 0, Double.POSITIVE_INFINITY }));
			Assert.fail("Expected MathIllegalArgumentException");
		} catch (final MathIllegalArgumentException e) {
		}
	}
	...
	public void testStep_0args() {
		...
		Assert.assertArrayEquals(transitionMatrix.getRow(0),
				MathArrays.normalizeArray(new double[] { c[0][0], c[0][1] }, 1), 1e-2);
		Assert.assertArrayEquals(transitionMatrix.getRow(1),
				MathArrays.normalizeArray(new double[] { c[1][0], c[1][1] }, 1), 1e-2);
		...
		Assert.assertArrayEquals(stationaryVector.toArray(),
				MathArrays.normalizeArray(new double[] { sc[0], sc[1] }, 1), 1e-2);
	}
	...
	public void testStep_int() {
		final MarkovChain walker = new MarkovChain(
				new Array2DRowRealMatrix(new double[][] { { 0, 1, 0 }, { 0, 0, 1 }, { 1, 0, 0 } }),
