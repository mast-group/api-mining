...
public class SequenceScaling {
	...
	public static void scalingTransactions(final int noCores, final int[] trans)
		...
		final Map<Sequence, Double> sequences = SequenceMiningCore.readISMSequences(itemsetLog);
		System.out.print("\n============= ACTUAL SEQUENCES =============\n");
		...
		transloop: for (int i = 0; i < trans.length; i++) {
			...
			TransactionGenerator.generateTransactionDatabase(sequences, tran, dbFile);
			...
			SequenceMining.mineSequences(dbFile, new InferGreedy(), maxStructureSteps, maxEMIterations, logFile);
